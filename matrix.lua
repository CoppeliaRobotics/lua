...
function Matrix:flip(dim)
    ...
    if dim == 1 then
        return Matrix(
                   self:rows(), self:cols(), function(i, j)
...
function Matrix:repmat(n, m)
    ...
    return Matrix(
               self:rows() * n, self:cols() * m, function(i, j)
...
function Matrix:nonhom()
    if self:rows() == 4 then
        return self:slice(1, 1, 3, self:cols()):applyfuncidx(
                   function(i, j, x)
                return x / self:get(4, j)
            end
               )
    else
        error('invalid shape')
...
function Matrix:gauss(jordan)
    assert(
        self:cols() >= self:rows(),
        'number of columns must be greater or equal to the number of rows'
    )
    local n = self:rows()
    local r = self:copy()
    local det = 1
...
function Matrix:print(name, opts)
    ...
    for i = 1, self:rows() do
        ...
        for j = 1, self:cols() do
            row = row ..
                      string.format('%' .. tostring(elemwidth[j]) .. 's', tostring(self:get(i, j)))
...
setmetatable(
    Matrix, {
        __call = function(self, rows, cols, data, t)
            ...
            local datagen, origdata = function()
                return 0
            end, data
...
if arg and #arg == 1 and arg[1] == 'test' then
    ...
    assert(
        m * m:t() * m * m:t() ==
            Matrix(
                3, 3, {
                    4330700, 7781700, 11232700, 7781700, 13982700, 20183700, 11232700, 20183700,
                    29134700,
                }
            )
    )
    assert(
        m:t() * m == Matrix(
            4, 4, {
                1523, 1586, 1649, 1712, 1586, 1652, 1718, 1784, 1649, 1718, 1787, 1856, 1712, 1784,
                1856, 1928,
            }
        )
    )
    ...
    assert(
        Matrix:where(Vector:range(5):lt(3), Vector {10, 20, 30, 40, 50}, Vector {5, 4, 3, 2, 1}) ==
            Vector {10, 20, 3, 2, 1}
    )
    ...
    rot_m = Matrix(
                3, 3, {
            0.0000000, -0.8660254, 0.5000000, 0.7071068, -0.3535534, -0.6123725, 0.7071068,
            0.3535534, 0.6123725,
        }
            )
    ...
    assert(
        approxEq(
            Matrix3x3:toquaternion(Matrix(3, 3, {-1, 0, 0, 0, -1, 0, 0, 0, 1})), {0, 0, 1, 0}
        )
    )
    assert(
        Matrix(2, 2, {1, 2, 3, 4}):kron(Matrix(2, 2, {0, 5, 6, 7})) ==
            Matrix(4, 4, {0, 5, 0, 10, 6, 7, 12, 14, 0, 15, 0, 20, 18, 21, 24, 28})
    )
    assert(
        Matrix(2, 3, {1, -4, 7, -2, 3, 3}):kron(
            Matrix(
                4, 4, {8, -9, -6, 5, 1, -3, -4, 7, 2, 8, -8, -3, 1, 2, -5, -1}
            )
        ) == Matrix(
            8, 12, {
    ...
    assert(
        approxEq(
            Vector:geomspace(1, 1000, 4, false), Vector {1, 5.62341325, 31.6227766, 177.827941}
        )
    )
    assert(approxEq(Vector:logspace(2, 3, 4), Vector {100, 215.443469, 464.15888336, 1000}))
    assert(
        approxEq(
            Vector:logspace(2, 3, 4, false), Vector {100, 177.827941, 316.22776602, 562.34132519}
        )
    )
    ...
    assert(
        Matrix:ones(2, 3):applyfuncidx(
            function(i, j, x)
                return 100 * x + 10 * i + j
            end
        ) == Matrix(2, 3, {111, 112, 113, 121, 122, 123})
    )
    assert(
        Matrix:ones(2, 3):applyfunc(
            function(x)
                return 100 * x
            end
        ) == Matrix(2, 3, {100, 100, 100, 100, 100, 100})
    )
    assert(
        Matrix:ones(2, 2):applyfunc2(
            Matrix:eye(2), function(x, y)
                return x - 2 * y
            end
        ) == Matrix(2, 2, {-1, 1, 1, -1})
    )
    ...
    assert(
        -22 ==
            Matrix(5, 5, {3, 2, 2, 1, 3, 0, 3, 0, 1, 3, 3, 0, 4, 3, 2, 2, 2, 1, 2, 2, 4, 3, 3, 1, 4}):det()
    )
    ...
    assert(
        Matrix(2, 3, {1, 2, 3, 10, 20, 30}):repmat(3, 2) == Matrix(
            6, 6, {
                1, 2, 3, 1, 2, 3, 10, 20, 30, 10, 20, 30, 1, 2, 3, 1, 2, 3, 10, 20, 30, 10, 20, 30,
