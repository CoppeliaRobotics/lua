...
require = wrap(require, function(origRequire)
    ...
    return function(...)
        ...
        if _DEVMODE then
            ...
            if resolved and lfsx.realpath(resolved) ~= lfsx.pathsanitize(resolved) then
...
function dump(x, maxDepth)
    ...
    if maxDepth > 0 and type(x) == 'table' then
        local mt = getmetatable(x)
        if mt and type(mt.__dump) == 'function' then
            return mt.__dump(x, maxDepth)
        end
        if mt == nil then
            local tbl = {}
