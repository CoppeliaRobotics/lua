...
function simRMLMoveToJointPositions(jhandles, flags, currentVel, currentAccel, maxVel, maxAccel,
    ...
    return sim.rmlMoveToJointPositions(
               jhandles, flags, currentVel, currentAccel, maxVel, maxAccel, maxJerk, targetPos,
               targetVel, direction
           )
end
...
function sim.rmlMoveToJointPositions(...)
    ...
    local jhandles, flags, currentVel, currentAccel, maxVel, maxAccel, maxJerk, targetPos,
          targetVel, direction = checkargs(
                                     {
            {type = 'table', size = '1..*', item_type = 'int'}, {type = 'int'},
            {type = 'table', size = '1..*', item_type = 'float', nullable = true},
            {type = 'table', size = '1..*', item_type = 'float', nullable = true},
    ...
    if dof < 1 or (currentVel and dof > #currentVel) or (currentAccel and dof > #currentAccel) or
        dof > #maxVel or dof > #maxAccel or dof > #maxJerk or dof > #targetPos or
        (targetVel and dof > #targetVel) or (direction and dof > #direction) then
    ...
    if direction == nil then
        direction = {}
        for i = 1, #jhandles, 1 do direction[i] = 0 end
    end
    ...
    local endPos, endVel, endAccel, timeLeft = sim.moveToConfig(
                                                   flags, currentConf, currentVel, currentAccel,
                                                   maxVel, maxAccel, maxJerk, targetPos, targetVel,
                                                   _S.tmpCb, jhandles, cycl
                                               )
    local res = 0
...
function simRMLMoveToPosition(handle, rel, flags, currentVel, currentAccel, maxVel, maxAccel,
                              maxJerk, targetPos, targetQuat, targetVel)
    -- Deprecated function, for backward compatibility (02.10.2020)
    return sim.rmlMoveToPose(
               handle, rel, flags, currentVel, currentAccel, maxVel, maxAccel, maxJerk, targetPos,
               targetQuat, targetVel
           )
end

function sim.rmlMoveToPosition(...)
    -- Deprecated function, for backward compatibility (02.10.2020)
    local handle, rel, flags, currentVel, currentAccel, maxVel, maxAccel, maxJerk, targetPos,
          targetQuat, targetVel = checkargs(
                                      {
            {type = 'int'}, {type = 'int'}, {type = 'int'},
            {type = 'table', size = 4, item_type = 'float', nullable = true},
            {type = 'table', size = 4, item_type = 'float', nullable = true},
    ...
    data.handle = handle
    data.rel = rel
    local endMatrix, timeLeft = sim.moveToPose(
                                    flags, mStart, maxVel, maxAccel, maxJerk, mGoal, _S.tmpCb, data
                                )
    local res = 0
    local nPos, nQuat
    if endMatrix then
...
function sim.setSimilarName(handle, original, suffix)
    ...
    while true do
        local nm = base
        if hash == '#' then
            if cnt >= 0 then nm = nm .. cnt end
            nm = nm .. '#' .. index
            newName = nm
            cnt = cnt + 1
        else
            if index >= 0 then nm = nm .. index end
            newName = nm
            nm = nm .. '#'
            index = index + 1
        end
        if sim.getObjectHandle(nm, {noError = true}) == -1 then break end
    end
...
function sim.include(relativePathAndFile, cmd)
    ...
    if not __notFirst__ then
        ...
        if __scriptCodeToRun__ then __scriptCodeToRun__() end
    end
end
...
function sim.canScaleObjectNonIsometrically(objHandle, scaleAxisX, scaleAxisY, scaleAxisZ)
    ...
    if t == sim.sceneobject_shape then
        ...
        if pt == sim.pure_primitive_spheroid then return false end
        if pt == sim.pure_primitive_disc then return xIsY end
        if pt == sim.pure_primitive_cylinder then return xIsY end
        if pt == sim.pure_primitive_cone then return xIsY end
        if pt == sim.pure_primitive_heightfield then return xIsY end
        return true
    end
end

function sim.canScaleModelNonIsometrically(modelHandle, scaleAxisX, scaleAxisY, scaleAxisZ,
                                           ignoreNonScalableItems)
    local xIsY = (math.abs(1 - math.abs(scaleAxisX / scaleAxisY)) < 0.001)
    local xIsZ = (math.abs(1 - math.abs(scaleAxisX / scaleAxisZ)) < 0.001)
        ...
        if ((t ~= sim.sceneobject_pointcloud) and (t ~= sim.sceneobject_pointcloud) and
            ...
            if matchingAxesCnt == 0 then
                ...
                if matchingAxesCnt == 3 then
                    if not sim.canScaleObjectNonIsometrically(
                        h, objFrameScalingFactors[1], objFrameScalingFactors[2],
                        objFrameScalingFactors[3]
                    ) then return false end
                else
                    ...
                    if (math.abs(1 - math.abs(otherFactors[1] / otherFactors[2])) < 0.001) then
                        ...
                        fff[singleMatchingAxisIndex] =
                            objFrameScalingFactors[singleMatchingAxisIndex]
...
function sim.UI_populateCombobox(ui, id, items_array, exceptItems_map, currentItem, sort,
                                 additionalItemsToTop_array)
    local _itemsTxt = {}
    local _itemsMap = {}
    ...
    if sort then table.sort(_itemsTxt) end
    local tableToReturn = {}
    if additionalItemsToTop_array then
        for i = 1, #additionalItemsToTop_array, 1 do
            tableToReturn[#tableToReturn + 1] = {
                additionalItemsToTop_array[i][1], additionalItemsToTop_array[i][2],
            }
        end
    end
...
function sim.displayDialog(...)
    local title, mainTxt, style, modal, initTxt, d1, d2, d3 = checkargs(
                                                                  {
    ...
    if sim.getBoolParam(sim.boolparam_headless) then return -1 end
    local retVal = -1
    local center = true
    ...
    if modal and style == sim.dlgstyle_message then modal = false end
    ...
    if not _S.dlg.nextHandle then _S.dlg.nextHandle = 0 end
    ...
    if modal then while _S.dlg.allDlgResults[retVal] == sim.dlgret_still_open do sim.step() end end
    return retVal
end
