...
function _S.conveyorSystem.init(config)
    ...
    sim.writeCustomBufferData(
        _S.conveyorSystem.model, 'CONVMOV', sim.packTable({currentPos = _S.conveyorSystem.offset})
    )
...
function sysCall_actuation()
    local dat = sim.readCustomBufferData(_S.conveyorSystem.model, 'CONVMOV')
    local off
    ...
    if off or _S.conveyorSystem.velocity ~= 0 then
        if off then
            ...
            _S.conveyorSystem.offset = _S.conveyorSystem.offset + _S.conveyorSystem.velocity *
        ...
        if _S.conveyorSystem.config.useRollers then
            for i = 1, #_S.conveyorSystem.rolHandles, 1 do
                sim.setJointPosition(
                    _S.conveyorSystem.rolHandles[i],
                    2 * _S.conveyorSystem.offset / _S.conveyorSystem.config.rollerSize[1]
                )
            end
        else
    ...
    sim.writeCustomBufferData(_S.conveyorSystem.model, 'CONVMOV', sim.packTable(dat))
end
...
function path.refreshTrigger(ctrlPts, pathData, config)
    ...
    if (config.bitCoded & 2) == 0 then
        -- open
        if _S.conveyorSystem.config.useRollers then
            rolCnt = 1 + _S.conveyorSystem.totalLength //
                         (_S.conveyorSystem.config.rollerSize[1] +
                             _S.conveyorSystem.config.interPadSpace)
            _S.conveyorSystem.totalL = _S.conveyorSystem.totalLength
            _S.conveyorSystem.padOffset = _S.conveyorSystem.totalLength / (rolCnt - 1)
        else
            padCnt = 1 + _S.conveyorSystem.totalLength //
                         (_S.conveyorSystem.config.padSize[1] +
                             _S.conveyorSystem.config.interPadSpace)
            _S.conveyorSystem.padOffset = _S.conveyorSystem.config.padSize[1] +
                                              _S.conveyorSystem.config.interPadSpace
        ...
        -- closed
        if _S.conveyorSystem.config.useRollers then
            rolCnt = _S.conveyorSystem.totalLength //
                         (_S.conveyorSystem.config.rollerSize[1] +
                             _S.conveyorSystem.config.interPadSpace)
            _S.conveyorSystem.padOffset = _S.conveyorSystem.totalLength / rolCnt
        else
            padCnt = _S.conveyorSystem.totalLength //
                         (_S.conveyorSystem.config.padSize[1] +
                             _S.conveyorSystem.config.interPadSpace)
    ...
    if padCnt == #oldPads and rolCnt == #oldJoints and sim.packTable(_S.conveyorSystem.config) ==
        sim.readCustomBufferData(_S.conveyorSystem.model, 'CONVEYORSET') then
        _S.conveyorSystem.padHandles = oldPads -- reuse old pads, they are the same
        _S.conveyorSystem.rolHandles = oldJoints
    else
        sim.writeCustomBufferData(
            _S.conveyorSystem.model, 'CONVEYORSET', sim.packTable(_S.conveyorSystem.config)
        )
        ...
        if _S.conveyorSystem.config.useRollers then
            for i = 1, rolCnt, 1 do
                local opt = 16
                if _S.conveyorSystem.config.respondablePads then opt = opt + 8 end
                local cyl = sim.createPureShape(
                                2, opt, {
                        _S.conveyorSystem.config.rollerSize[1],
                        _S.conveyorSystem.config.rollerSize[1],
                ...
                sim.setShapeColor(
                    cyl, nil, sim.colorcomponent_ambient_diffuse, _S.conveyorSystem.config.padCol
                )
                ...
                local pos = sim.getPathInterpolatedConfig(
                                _S.conveyorSystem.pathPositions, _S.conveyorSystem.pathLengths, o
                            )
                pos[3] = pos[3] - _S.conveyorSystem.config.rollerSize[1] / 2
                local quat = sim.getPathInterpolatedConfig(
            ...
            for i = 1, padCnt, 1 do
                local opt = 16
                if _S.conveyorSystem.config.respondablePads then opt = opt + 8 end
                _S.conveyorSystem.padHandles[i] = sim.createPureShape(
                                                      0, opt, {
                        _S.conveyorSystem.config.padSize[1],
                        _S.conveyorSystem.config.padSize[2] * 0.95,
                ...
                sim.setShapeColor(
                    ...
                    _S.conveyorSystem.config.padCol
                )
                ...
                sim.setObjectInt32Param(
                    _S.conveyorSystem.padHandles[i], sim.objintparam_visibility_layer, 1 + 256
                )
            end
        end
        if _S.conveyorSystem.config.respondableBase then
            local cnt = 1 + _S.conveyorSystem.totalLength //
                            (_S.conveyorSystem.config.respondableBaseElementLength * 0.5)
            local off = _S.conveyorSystem.config.respondableBaseElementLength * 0.5
            local el = {}
            local p = 0
            if _S.conveyorSystem.config.useRollers then
                for i = 1, cnt, 1 do
                    el[i] = sim.createPureShape(
                                0, 24, {
                            _S.conveyorSystem.config.respondableBaseElementLength,
                            _S.conveyorSystem.config.rollerSize[2],
                    ...
                    local quat = sim.getPathInterpolatedConfig(
                                     _S.conveyorSystem.pathQuaternions,
                                     _S.conveyorSystem.pathLengths, p, nil, {2, 2, 2, 2}
                                 )
        ...
        if _S.conveyorSystem.config.useBorder then
            local cnt = _S.conveyorSystem.totalLength //
                            (_S.conveyorSystem.config.borderSize[1] * 0.5)
            ...
            for i = 1, cnt2, 1 do
                ...
                sim.setShapeColor(
                    pa, nil, sim.colorcomponent_ambient_diffuse, _S.conveyorSystem.config.col
                )
                local pb = sim.createPureShape(0, 24, _S.conveyorSystem.config.borderSize, 0.01)
                sim.setShapeColor(
                    pb, nil, sim.colorcomponent_ambient_diffuse, _S.conveyorSystem.config.col
                )
                sim.setObjectPosition(pa, {0, (w - _S.conveyorSystem.config.borderSize[2]) / 2, 0})
                sim.setObjectPosition(pb, {0, -(w - _S.conveyorSystem.config.borderSize[2]) / 2, 0})
                el[i] = sim.groupShapes({pa, pb})
                sim.reorientShapeBoundingBox(el[i], -1)
                local pos = sim.getPathInterpolatedConfig(
                                _S.conveyorSystem.pathPositions, _S.conveyorSystem.pathLengths, p
                            )
                if _S.conveyorSystem.config.useRollers then
                    pos[3] = pos[3] - _S.conveyorSystem.config.rollerSize[1] / 2 +
                                 _S.conveyorSystem.config.borderSize[3] / 2
                else
                    pos[3] = pos[3] - _S.conveyorSystem.config.padSize[3] +
                                 _S.conveyorSystem.config.borderSize[3] / 2
                end
                local quat = sim.getPathInterpolatedConfig(
                                 _S.conveyorSystem.pathQuaternions, _S.conveyorSystem.pathLengths,
...
function path.shaping(path, pathIsClosed, upVector)
    ...
    if _S.conveyorSystem.config.useRollers then
        ...
        section = {
            -_S.conveyorSystem.config.rollerSize[2] / 2, -_S.conveyorSystem.config.rollerSize[1],
            -_S.conveyorSystem.config.rollerSize[2] / 2,
            -_S.conveyorSystem.config.rollerSize[1] / 2, _S.conveyorSystem.config.rollerSize[2] / 2,
            -_S.conveyorSystem.config.rollerSize[1] / 2, _S.conveyorSystem.config.rollerSize[2] / 2,
            -_S.conveyorSystem.config.rollerSize[1], -_S.conveyorSystem.config.rollerSize[2] / 2,
            -_S.conveyorSystem.config.rollerSize[1],
        }
    else
        --  section={-_S.conveyorSystem.config.padSize[2]/2,-_S.conveyorSystem.config.padSize[2]/2-_S.conveyorSystem.config.padSize[3],-_S.conveyorSystem.config.padSize[2]/2,-_S.conveyorSystem.config.padSize[3],_S.conveyorSystem.config.padSize[2]/2,-_S.conveyorSystem.config.padSize[3],_S.conveyorSystem.config.padSize[2]/2,-_S.conveyorSystem.config.padSize[2]/2-_S.conveyorSystem.config.padSize[3],-_S.conveyorSystem.config.padSize[2]/2,-_S.conveyorSystem.config.padSize[2]/2-_S.conveyorSystem.config.padSize[3]}
        section = {
            -_S.conveyorSystem.config.padSize[2] / 2, -0.02 - _S.conveyorSystem.config.padSize[3],
            -_S.conveyorSystem.config.padSize[2] / 2, -_S.conveyorSystem.config.padSize[3],
            _S.conveyorSystem.config.padSize[2] / 2, -_S.conveyorSystem.config.padSize[3],
            _S.conveyorSystem.config.padSize[2] / 2, -0.02 - _S.conveyorSystem.config.padSize[3],
            -_S.conveyorSystem.config.padSize[2] / 2, -0.02 - _S.conveyorSystem.config.padSize[3],
        }
...
function _S.conveyorSystem.setPathPos(p)
    for i = 1, #_S.conveyorSystem.padHandles, 1 do
        ...
        local o = p
        if o > _S.conveyorSystem.totalLength then o = o - _S.conveyorSystem.padOffset end
        local pos = sim.getPathInterpolatedConfig(
                        _S.conveyorSystem.pathPositions, _S.conveyorSystem.pathLengths, o
                    )
        pos[3] = pos[3] - _S.conveyorSystem.config.padSize[3] / 2
        local quat = sim.getPathInterpolatedConfig(
                         _S.conveyorSystem.pathQuaternions, _S.conveyorSystem.pathLengths, o, nil,
                         {2, 2, 2, 2}
                     )
        ...
        if pp[1] > _S.conveyorSystem.config.padSize[2] or pp[2] >
            _S.conveyorSystem.config.padSize[2] or pp[3] > _S.conveyorSystem.config.padSize[2] then
