...
function AuxiliaryConsoleOpen(...)
    ...
    return sim.auxiliaryConsoleOpen(
...
function AddDrawingObject_spheres(...)
    ...
    local obj = sim.addDrawingObject(
                    sim.drawing_spherepoints, size, 0, -1, 10000, adCol, nil, nil, eCol
                )
    for i = 0, math.floor(#coords / 3) - 1, 1 do
        sim.addDrawingObjectItem(obj, {coords[3 * i + 1], coords[3 * i + 2], coords[3 * i + 3]})
    end
...
function AddDrawingObject_cubes(...)
    ...
    local obj = sim.addDrawingObject(
                    sim.drawing_cubepoints, size, 0, -1, 10000, adCol, nil, nil, eCol
                )
    for i = 0, math.floor(#coords / 3) - 1, 1 do
        sim.addDrawingObjectItem(
            obj, {coords[3 * i + 1], coords[3 * i + 2], coords[3 * i + 3], 0, 0, 1}
        )
    end
    return obj
end
...
function AddDrawingObject_segments(...)
    ...
    local obj = sim.addDrawingObject(
                    sim.drawing_lines, lineSize, 0, -1, 10000, adCol, nil, nil, eCol
                )
    for i = 0, math.floor(#segments / 6) - 1, 1 do
        sim.addDrawingObjectItem(
            obj, {
                segments[6 * i + 1], segments[6 * i + 2], segments[6 * i + 3], segments[6 * i + 4],
                segments[6 * i + 5], segments[6 * i + 6],
            }
        )
    end
    return obj
...
function AddDrawingObject_triangles(...)
    ...
    for i = 0, math.floor(#triangles / 9) - 1, 1 do
        sim.addDrawingObjectItem(
            obj, {
                triangles[9 * i + 1], triangles[9 * i + 2], triangles[9 * i + 3],
                triangles[9 * i + 4], triangles[9 * i + 5], triangles[9 * i + 6],
                triangles[9 * i + 7], triangles[9 * i + 8], triangles[9 * i + 9],
            }
        )
    end
    return obj
...
function CallScriptFunction(...)
    ...
    if type(scriptType) == 'string' then scriptType = evalStr(scriptType) end
...
function CheckCollision(...)
    ...
    if type(entity2) == 'string' then entity2 = evalStr(entity2) end
...
function CheckProximitySensor(...)
    ...
    local sensor, entity = ...
    if type(entity) == 'string' then entity = evalStr(entity) end
...
function CheckVisionSensor(...)
    ...
    local sensor, entity = ...
    if type(entity2) == 'string' then entity = evalStr(entity) end
...
function GetObjectPosition(...)
    ...
    if type(relObjHandle) == 'string' then relObjHandle = evalStr(relObjHandle) end
...
function GetObjectHandle(...)
    ...
    local objName = ...
    if (string.find(objName, '/') == nil) and (string.find(objName, '.') == nil) and
        (string.find(objName, ':') == nil) then
        -- Old way of accessing objects
        if string.find(objName, '#') == nil then objName = objName .. '#' end
    end
...
function GetVisionSensorImage(...)
    ...
    if greyScale then objHandle = objHandle + sim.handleflag_greyscale end
...
function GetVisionSensorDepthBuffer(...)
    ...
    if toMeters then objHandle = objHandle + sim.handleflag_depthbuffermeters end
    if asByteArray then objHandle = objHandle + sim.handleflag_codedstring end
...
function GetObjectName(...)
    ...
    if altName then handle = handle + sim.handleflag_altname end
...
function SetStringParam(...)
    ...
    local paramId, val = ...
    if type(paramId) == 'string' then paramId = evalStr(paramId) end
...
function DisplayDialog(...)
    ...
    if type(dlgType) == 'string' then dlgType = evalStr(dlgType) end
...
function GetCollisionHandle(...)
    ...
    local name = ...
    if string.find(name, '#') == nil then name = name .. '#' end
...
function SetObjectParent(...)
    ...
    if assembly then handle = handle + sim.handleflag_assembly end
...
function GetObjectsInTree(...)
    ...
    if type(treeBase) == 'string' then treeBase = evalStr(treeBase) end
    if type(objType) == 'string' then objType = evalStr(objType) end
...
function timeStr()
    ...
    if sim.getSimulationState() == sim.simulation_stopped then
        ...
        t =
            string.format('[%02d:%02d:%02d.%02d] ', t.hour, t.min, t.sec, math.floor(100 * (st % 1)))
    end
    return t
end
...
function Synchronous(...)
    ...
    local enable = ...
    if enable and not syncMode then syncModeWait = true end
    syncMode = enable
    return true
end
...
function debugFunc(funcName, ...)
    ...
    if modelData and modelData.debugLevel >= 3 then
        ...
        if arg == '' then arg = 'none' end
        local a =
            timeStr() .. " calling function '" .. funcName .. "' with following arguments: " .. arg
...
function PCALL(func, printErrors, ...)
    ...
    if modelData and modelData.debugLevel >= 1 and (not res) and printErrors then
        local a = string.format(
                      timeStr() .. " error while calling function '%s': %s", lastFuncName, a
                  )
        sim.addLog(sim.verbosity_msgs, a)
    end
...
function PACKPUBMSG(topic, res, ...)
    local a = {...}
    local b = {topic, {res}}
    for i = 1, #a, 1 do b[2][#b[2] + 1] = a[i] end
...
function createNode()
    if not b0Node then
        if modelData.debugLevel >= 1 then
            local a = string.format(
                          timeStr() ..
                              " creating BlueZero node '%s' and associated publisher, subscriber and service server (on channel '%s')",
                          modelData.nodeName, modelData.channelName
                      )
            sim.addLog(sim.verbosity_msgs, a)
        end
        ...
        if not initStg then
            ...
            if simUI then
                local xml =
                    [[ <ui closeable="false" resizable="false" title="BlueZero" modal="true">
            ...
            if simUI then simUI.destroy(ui) end
        ...
        if initStg == 1 then
            ...
            serviceServer = simB0.serviceServerCreate(
                                b0Node, modelData.channelName .. 'SerX', 'serviceServer_callback'
                            )
            defaultPublisher = simB0.publisherCreate(b0Node, modelData.channelName .. 'PubX')
            defaultSubscriber = simB0.subscriberCreate(
                                    b0Node, modelData.channelName .. 'SubX',
                                    'defaultSubscriber_callback'
                                )
...
function destroyNode()
    if b0Node then
        if modelData.debugLevel >= 1 then
            local a = string.format(
                          timeStr() ..
                              " destroying BlueZero node '%s' and associated publisher, subscriber and service server (on channel '%s')",
                          modelData.currentNodeName, modelData.currentChannelName
                      )
            sim.addLog(sim.verbosity_msgs, a)
        end
        ...
        local clients = {}
        for key, val in pairs(allClients) do clients[#clients + 1] = key end
        for i = 1, #clients, 1 do DisconnectClient(clients[i]) end
...
function sendAndSpin(calledMoment)
    ...
    if b0Node then
            ...
            if not hasClientReachedMaxInactivityTime(clientId) then
                ...
                for topic, value in pairs(val) do
                    ...
                    for i = 1, #cmdList, 1 do
                        local cmd = cmdList[i]
                        if triggerInterval == 0 or calledMoment == 0 or
                            (nextSimulationStepUnderway and
                                not (sim.getSimulationState() == sim.simulation_paused)) or cmd.func ==
                            'GetSimulationStepStarted' then
                            ...
                            if cmd.triggerIntervalCnt <= 0 then
                                ...
                                local retVal = PACKPUBMSG(
                                                   topic,
                                                   PCALL(_G[cmd.func], true, unpack(cmd.args))
                                               )

                                if cmd.func == 'GetSimulationStepDone' then
                                    publishSimulationStepFinished_ClientAnddata[clientId] = {
                                        publisher, retVal,
                                    } -- publish this one last! (further down)
                                else
                                    if cmd.func == 'GetSimulationStepStarted' then
                                        publishSimulationStepStarted_ClientAnddata[clientId] = {
                                            publisher, retVal,
                                        } -- publish this one last! (further down)
                                    else
                                        publish(
                                            clientId, publisher, cmd.func, retVal,
                                            publisherCntForClients
                                        )
                                    end
                                end
                            end
        ...
        for i = 1, #clientsToRemove, 1 do
            ...
            if modelData.debugLevel >= 1 then
                local a = string.format(
                              timeStr() ..
                                  " destroyed all streaming functions for client '%s' after detection of inactivity",
                              clientId
                          )
                sim.addLog(sim.verbosity_msgs, a)
            end
    ...
    for publisher, client in pairs(publisherCntForClients) do
        ...
        if msgCnt > 0 and modelData and modelData.debugLevel >= 2 then
            local a = string.format(
                          timeStr() .. " published %i message(s) to %i client(s): %s", msgCnt,
                          clientCnt, funcs
                      )
            sim.addLog(sim.verbosity_msgs, a)
        end
...
function publish(clientId, publisher, func, retVal, publisherCntForClients)
    ...
    if not publisherCntForClients[publisher] then publisherCntForClients[publisher] = {} end

    if not publisherCntForClients[publisher][clientId] then
        publisherCntForClients[publisher][clientId] = {cnt = 1, funcs = func}
    else
        publisherCntForClients[publisher][clientId].cnt =
            publisherCntForClients[publisher][clientId].cnt + 1
        publisherCntForClients[publisher][clientId].funcs =
            publisherCntForClients[publisher][clientId].funcs .. '|' .. func
    end
end

function updateClientLastActivityTime(clientId)
    if not allClients[clientId] then allClients[clientId] = {maxInactivityTimeMs = 60 * 1000} end
...
function hasClientReachedMaxInactivityTime(clientId)
    local val = allClients[clientId]
    if val.maxInactivityTimeMs == 0 then return false end
...
function serviceServer_callback(receiveMsg)
    ...
    if not handlePublisherSetupFunctions(task, funcName, clientId, topic, funcArgs) then
        if funcName == 'createSubscriber' then
            local subscr = simB0.subscriberCreate(
                               b0Node, funcArgs[1], 'dedicatedSubscriber_callback', false, true
                           )
            ...
            dedicatedSubscribers[clientId][funcArgs[1]] = {
                handle = subscr,
                dropMessages = funcArgs[2],
            }
            if modelData.debugLevel >= 1 then
                local a = string.format(
                              timeStr() ..
                                  " creating dedicated subscriber for client '%s' with topic '%s'",
                              clientId, funcArgs[1]
                          )
                sim.addLog(sim.verbosity_msgs, a)
            end
        elseif funcName == 'inactivityTolerance' then
            ...
            if modelData.debugLevel >= 2 then
                local a = string.format(
                              timeStr() .. " setting max. inactivity tolerance for client '%s'",
                              clientId
                          )
                sim.addLog(sim.verbosity_msgs, a)
            end
            ...
            if modelData.debugLevel >= 2 then
                local a = string.format(
                              timeStr() .. " called function for client '%s': %s (service call)",
                              clientId, receiveMsg[1][1]
                          )
                sim.addLog(sim.verbosity_msgs, a)
            end
        end
...
function handlePublisherSetupFunctions(task, funcName, clientId, topic, funcArgs)
    if task == 2 then
        ...
        if not allPublishers[clientId] then allPublishers[clientId] = {} end
        if not allPublishers[clientId][topic] then
            allPublishers[clientId][topic] = {
                handle = defaultPublisher,
                cmds = {},
                triggerInterval = 1,
            }
        end
        ...
        if modelData.debugLevel >= 1 then
            local a = string.format(
                          timeStr() ..
                              " registering streaming function '%s' for client '%s' on topic '%s' (default publisher)",
                          funcName, clientId, topic
                      )
            sim.addLog(sim.verbosity_msgs, a)
        end
    elseif task == 4 then
        ...
        if allPublishers[clientId] and allPublishers[clientId][topic] then
            ...
            if modelData.debugLevel >= 1 then
                local a = string.format(
                              timeStr() ..
                                  " registering streaming function '%s' for client '%s' on topic '%s' (dedicated publisher)",
                              funcName, clientId, topic
                          )
                sim.addLog(sim.verbosity_msgs, a)
            end
        ...
        if funcName == 'createPublisher' then
            ...
            allPublishers[clientId][targetTopic] = {
                handle = pub,
                cmds = {},
                triggerInterval = trigInterv,
            }
            if modelData.debugLevel >= 1 then
                local a = string.format(
                              timeStr() ..
                                  " creating dedicated publisher for client '%s' with topic '%s'",
        ...
        elseif funcName == 'setDefaultPublisherPubInterval' then
            ...
            if modelData.debugLevel >= 2 then
                local a = string.format(
                              timeStr() ..
                                  " setting default publisher interval for client '%s' with topic '%s'",
                              clientId, targetTopic
                          )
                sim.addLog(sim.verbosity_msgs, a)
            end
            ...
            if allPublishers[clientId] then
                if allPublishers[clientId][topic] then
                    ...
                    if modelData.debugLevel >= 1 then
                        ...
                        if #cmds > 0 then
                            cm =
                                '. Following streaming functions on that topic will be unregistered:'
                            for i = 1, #cmds, 1 do
                                if i == 1 then
                                    cm = cm .. ' '
                                else
                        ...
                        local a = string.format(
                                      timeStr() ..
                                          " stopping %s publisher for client '%s' with topic '%s'. All Streaming functions on that topic will be unregistered%s",
                                      nn, clientId, topic, cm
                                  )
                        sim.addLog(sim.verbosity_msgs, a)
                    end
                end
            ...
            if dedicatedSubscribers[clientId] then
                if dedicatedSubscribers[clientId][topic] then
                    ...
                    if modelData.debugLevel >= 1 then
                        local a = string.format(
                                      timeStr() ..
                                          " stopping dedicated subscriber for client '%s' with topic '%s'",
                                      clientId, topic
                                  )
                        sim.addLog(sim.verbosity_msgs, a)
                    end
...
function defaultSubscriber_callback(msg)
    ...
    if not handlePublisherSetupFunctions(task, funcName, clientId, topic, funcArgs) then
        ...
        if modelData.debugLevel >= 2 then
            local a = string.format(
                          timeStr() .. " called function for client '%s': %s (default subscriber)",
                          clientId, funcName
                      )
            sim.addLog(sim.verbosity_msgs, a)
        end
...
function dedicatedSubscriber_callback(msg)
    ...
    if modelData.debugLevel >= 2 then
        local a = string.format(
                      timeStr() .. " called function for client '%s': %s (dedicated subscriber)",
                      clientId, funcName
                  )
        sim.addLog(sim.verbosity_msgs, a)
    end
...
function onConfigNodeNameChanged(ui, id, newVal)
    if #newVal > 2 then
        ...
        for i = 1, #newVal, 1 do
            ...
            if (v >= '0' and v <= '9') or (v >= 'a' and v <= 'z') or (v >= 'A' and v <= 'Z') or v ==
                '_' or v == '-' then
                newValue = newValue .. v
            else
                newValue = newValue .. '_'
...
function sysCall_init()
    ...
    if model == -1 then
        ...
        model = -1
        modelData = {
            nodeName = 'b0RemoteApi_CoppeliaSim-addOn',
            channelName = 'b0RemoteApiAddOn',
            debugLevel = 1,
            packStrAsBin = false,
            duringSimulationOnly = false,
        }
    else
    ...
    syncMode = false
    if not abort then createNode() end
end
...
function sysCall_beforeMainScript()
    if not sendAndSpin(1) then return {doNotRunMainScript = true} end
end
...
function sysCall_beforeSimulation()
    if modelData.duringSimulationOnly then createNode() end
end

function sysCall_afterSimulation()
    if modelData.duringSimulationOnly then destroyNode() end
    syncMode = false
end

function sysCall_beforeInstanceSwitch()
    if model >= 0 then destroyNode() end
end

function sysCall_afterInstanceSwitch()
    if model >= 0 then
        if not modelData.duringSimulationOnly then createNode() end
        createNode()
    end
end
...
function sysCall_userConfig()
    ...
    local xml =
        '<ui title="BlueZero-based remote API, server-side configuration" closeable="true" on-close="onDlgClose" modal="true" resizable="false" activate="false" enabled="' ..
            tostring(simStopped)
    xml = xml .. [[">
