...
function string.formatex(fmt, ...)
    local arg1 = {}
    for i, v in ipairs(arg) do arg1[i] = (type(v) == 'table' and table.val_to_str(v) or v) end
...
function createOpenBox(size, baseThickness, wallThickness, density, inertiaCorrectionFact, static,
                       respondable, color)
    local parts = {}
    ...
    sim.setObjectPosition(
        parts[2], {(size[1] - wallThickness) * 0.5, 0, baseThickness + dim[3] * 0.5}
    )
    parts[3] = sim.createPureShape(0, 16, dim, density * dim[1] * dim[2] * dim[3])
    sim.setObjectPosition(
        parts[3], {(-size[1] + wallThickness) * 0.5, 0, baseThickness + dim[3] * 0.5}
    )
    ...
    sim.setObjectPosition(
        parts[4], {0, (size[2] - wallThickness) * 0.5, baseThickness + dim[3] * 0.5}
    )
    parts[5] = sim.createPureShape(0, 16, dim, density * dim[1] * dim[2] * dim[3])
    sim.setObjectPosition(
        parts[5], {0, (-size[2] + wallThickness) * 0.5, baseThickness + dim[3] * 0.5}
    )
    for i = 1, #parts, 1 do
    ...
    if math.abs(1 - inertiaCorrectionFact) > 0.001 then
        ...
        for i = 1, #i0, 1 do i0[i] = i0[1] * inertiaCorrectionFact end
...
function customUi_populateCombobox(ui, id, items_array, exceptItems_map, currentItem, sort,
...
function stringToArray(txt)
    ...
    if f then
        local res, arr = xpcall(
                             f, function(err)
                return debug.traceback(err)
            end
                         )
        if res then retVal = arr end
    end
    return retVal
end
...
function executeCode(theCode)
    ...
    if f then
        local res, theReturn = xpcall(
                                   f, function(err)
                return debug.traceback(err)
            end
                               )
        if res then return theReturn end
    end
end
...
function canScaleModelNonIsometrically(modelHandle, scaleAxisX, scaleAxisY, scaleAxisZ,
        ...
        if ((t ~= sim.sceneobject_pointcloud) and (t ~= sim.sceneobject_pointcloud) and
            ...
            if matchingAxesCnt == 0 then
                ...
                if matchingAxesCnt == 3 then
                    if not canScaleObjectNonIsometrically(
...
function utils.createCustomUi(nakedXml, title, dlgPos, closeable, onCloseFunction, modal, resizable,
                              activate, additionalAttributes, dlgSize)
    ...
    if dlgSize then xml = xml .. ' size="' .. dlgSize[1] .. ',' .. dlgSize[2] .. '"' end
    if not dlgPos then
        xml = xml .. ' placement="relative" position="-50,50">'
    else
        if type(dlgPos) == 'string' then
            if dlgPos == 'center' then xml = xml .. ' placement="center">' end
...
function utils.writeSessionPersistentObjectData(objectHandle, dataName, ...)
    local data = {...}
    local nm = "___" .. sim.getScriptAttribute(sim.handle_self, sim.scriptattribute_scripthandle) ..
