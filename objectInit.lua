    ...
    elseif objectType == 'shape' then
        ...
        if p.mesh then
            ...
            if type(p.mesh.texture) == 'table' then
                ...
                texture_interpolate = objInit.extractValueOrDefault('interpolate', true, p.mesh.texture)
                texture_decal = objInit.extractValueOrDefault('decal', false, p.mesh.texture)
                texture_rgba = objInit.extractValueOrDefault('rgba', false, p.mesh.texture)
                texture_horizFlip = objInit.extractValueOrDefault('horizFlip', false, p.mesh.texture)
                texture_vertFlip = objInit.extractValueOrDefault('vertFlip', false, p.mesh.texture)
                texture_res = objInit.extractValueOrDefault('resolution', nil, p.mesh.texture)
                texture_coord = objInit.extractValueOrDefault('coordinates', nil, p.mesh.texture)
                texture_img = objInit.extractValueOrDefault('image', nil, p.mesh.texture)
            end
            local options = 0
            ...
            local vertices = objInit.extractValueOrDefault('vertices', nil, p.mesh)
            local indices = objInit.extractValueOrDefault('indices', nil, p.mesh)
            local normals = objInit.extractValueOrDefault('normals', nil, p.mesh)
            if normals then
            ...
            local bbQuat = objInit.extractValueOrDefault('boundingBoxQuaternion', nil, p.mesh)
            if bbQuat then
                h:alignBoundingBox(bbQuat)
            else
                h:alignBoundingBox({0.0, 0.0, 0.0, 0.0}) -- to encompass shape closest
            end
            local frameOrigin = objInit.extractValueOrDefault('frameOrigin', nil, p.mesh)
            if frameOrigin then
        ...
        elseif p.heightField then
            ...
            local options = 0
                + v(1, objInit.extractValueOrDefault('culling'))
                + v(2, objInit.extractValueOrDefault('showEdges'))
                + v(4, objInit.extractValueOrDefault('rawMesh', false, p.heightField))
            local shadingAngle = objInit.extractValueOrDefault('shadingAngle')
            local heights = objInit.extractValueOrDefault('heights', nil, p.heightField)
            local cellSize = objInit.extractValueOrDefault('cellSize', nil, p.heightField)
        ...
        elseif p.string then
            ...
            local text = objInit.extractValueOrDefault('text', nil, p.string)
            local height = objInit.extractValueOrDefault('height', nil, p.string)
            local center = objInit.extractValueOrDefault('center', nil, p.string)
        ...
        h.dynamic = objInit.extractValueOrDefault('dynamic', false)
        if objInit.extractValueOrDefault('showEdges') then
            h:applyShowEdges(true)
        end
        h.applyColor.diffuse = objInit.extractValueOrDefault('color.diffuse')
        h.applyColor.specular = objInit.extractValueOrDefault('color.specular')
        h.applyColor.emission = objInit.extractValueOrDefault('color.emission')
